...
void
AnimatedSprite::SetAnimating(bool animate)
{
	m_bAnimating = animate;
}
void
...
void
AnimatedSprite::ResetBounds()
{
	min = 0;
	max = m_iTotalFrames;
}
void
AnimatedSprite::SetFrameBounds(int min, int max)
{
	this->min = min;
	this->max = max;
	m_iCurrentFrame = min;
...
void
AnimatedSprite::SetFrame(int frame)
{
